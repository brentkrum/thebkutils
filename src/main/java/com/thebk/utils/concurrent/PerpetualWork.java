...
public abstract class PerpetualWork
{
	...
	public PerpetualWork() {
		this(DefaultSystems.taskExecutor(), 1);
	}

	public PerpetualWork(int maxNumWorkPerRun) {
		this(DefaultSystems.taskExecutor(), maxNumWorkPerRun);
	}

	public PerpetualWork(Executor executor) {
		this(executor, 1);
	}

	public PerpetualWork(Executor executor, int maxNumWorkPerRun) {
	...
	private final class Runner implements Runnable {
		...
		private boolean run0(int numProcessed) {
			final Thread saveWorker = m_currentWorker;
