...
public class RCPromiseCombiner extends AbstractReferenceCounted {
	...
	private final RCFutureListenerWithParams<?> m_listener = new RCFutureListenerWithParams<Object>() {
		@Override
	...
	public void add(RCFuture f) {
		...
		retain();
		f.addListener(m_listener, ParamDataBag.create().po(m_current));
		m_numAdded++;
	}
	...
	public void finished(IRCPromiseCombinerCallback callback, ParamDataBag bag) {
		m_callbackBag = bag;
		m_callback = callback;
