...
abstract class TheBKPromise<T> implements RCPromise<T> {
    ...
    private static final int MAX_NOTIFY_DEPTH = Integer.parseInt(System.getProperty("DefaultRCPromise.max-notify-depth", "8"));
    private static final Logger LOG = LoggerFactory.getLogger(TheBKPromise.class);
    private static final AtomicReferenceFieldUpdater<TheBKPromise, Object> RESULT_UPDATER = AtomicReferenceFieldUpdater.newUpdater(TheBKPromise.class, Object.class, "m_result");
    private static final AtomicIntegerFieldUpdater<TheBKPromise> NOTIFYING_UPDATER = AtomicIntegerFieldUpdater.newUpdater(TheBKPromise.class, "m_notifyingListeners");
	...
	public RCFuture<T> addListener(RCFutureListenerWithParams<T> listener, ParamBag params) {
		...
		w.init((TheBKPromise)retain(), listener, params);
		m_listeners.add(w);
    ...
    public RCFuture<T> addListener(RCFutureListener<T> listener) {
        ...
        w.init((TheBKPromise)retain(), listener);
        m_listeners.add(w);
        // It is possible to have another thread complete and then notify BEFORE we added ourselves
        // to the listeners queue
        if (m_result != null) {
    ...
    protected void notifyListener0(RCFutureListener<T> l) {
        try {
    ...
    protected void notifyListener0(RCFutureListenerWithParams<T> l, ParamBag params) {
        try {
    ...
    private static class ListenerWrapper<T> extends ListenerWrapperBase<T> {
        ...
        void init(TheBKPromise<T> promise, RCFutureListener<T> listener) {
    ...
    private static class ListenerWrapperWithParams<T> extends ListenerWrapperBase<T> {
        ...
        void init(TheBKPromise<T> promise, RCFutureListenerWithParams<T> listener, ParamBag params) {
            this.promise = promise;
            this.listener = listener;
            this.params = params;
        }
