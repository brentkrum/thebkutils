...
public class Pipe {
	...
	private final TheBKQueue m_queue = new SPSCFixedQueue(1024);
	...
	private void close() {
		if (m_refCount.decrementAndGet() == 0) {
			...
			while(true) {
				ByteBuf buf = (ByteBuf)m_queue.dequeue();
				if (buf == null) {
					break;
				}
				buf.release();
	...
	private static class Consumer implements IConsumer {
		...
		public ByteBuf poll() {
			...
			return (ByteBuf)m_pipe.m_queue.dequeue();
		}
