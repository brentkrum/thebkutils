	...
	public void test() throws InterruptedException {
		SocketServer socketServer = SocketServer.builder(10000)
			.onData((conn, inPipe, outPipe) -> {
				...
				buf.release();
				LogManager.getLogger(SocketClient_ConnectToServer_Test.class).info("[{}] onData(\"{}\")", conn.remoteHostAddress(), request);
		...
		SocketClient client = SocketClient.builder("localhost", 10000).onConnect((conn, outPipe) -> {
			...
			buf.release();
			LogManager.getLogger(SocketClient_ConnectToServer_Test.class).info("onData(\"{}\")", response);
