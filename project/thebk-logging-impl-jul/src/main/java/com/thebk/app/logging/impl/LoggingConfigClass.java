...
import com.thebk.app.jul.DenaliAIFormatter;
import com.thebk.app.jul.DenaliConsoleHandler;
import com.thebk.app.jul.fmt.*;
...
public class LoggingConfigClass {
	...
	private void init() {
		...
		if (Config.getBoolean("com.thebk.app.disable-console", Boolean.FALSE) == false) {
		...
		try {
			String logDir = Config.getString("com.thebk.app.log-dir", "logs");
			String logFilePrefix = Config.getString("com.thebk.app.log-file-prefix", "app");
			String logFilePattern = logDir + "/" + logFilePrefix + "-%g" + ".log";
			int maxFileSize = Config.getInt("com.thebk.app.log-max-file-size-bytes", 5*1024*1024);
			int maxFileCount = Config.getInt("com.thebk.app.log-max-file-count", 10);
		...
		root.setLevel(Level.parse(Config.getString("com.thebk.app.logger-level-root", Level.INFO.getName())));
		...
		Level fwLevel = Level.parse(Config.getString("com.thebk.app.logger-level-fw", Level.INFO.getName()));
		...
		l.setLevel(fwLevel);
		m_keepLoggersAlive.add(l);
		l = Logger.getLogger("com.thebk.app");
		l.setLevel(fwLevel);
		m_keepLoggersAlive.add(l);
